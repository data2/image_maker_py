...
"中集车辆"]
    for hitKey in hitKeys:
        if hitKey in lineText:
            arr = lineText.split(hitKey)
            return True, [arr[0], hitKey, arr[1]]
    return False, []
...
def pic_text(filepath, size, text, setFont, fillColor, filename, direction=None):
    ...
    while True:
        if seq * lineWordCount >= len(text):
            ...
            lineText = text[(seq-1)*lineWordCount: seq * lineWordCount]
            flag, arr = hit(lineText)
            if flag:
                draw.text((size[0], size[1] + heightBetweenLine * (seq - 1)), arr[0],
                          font=setFont, fill=fillColor, direction=None)
                draw.text((size[0] + len(arr[0]) * 30, size[1] + heightBetweenLine * (seq - 1)), arr[1],
                          font=setFont, fill="green", direction=None)
                draw.text((size[0] + (len(arr[0]) + len(arr[1])) * 30, size[1] + heightBetweenLine * (seq - 1)), arr[2],
                          font=setFont, fill=fillColor, direction=None)
            else:
                draw.text((size[0], size[1] + heightBetweenLine * (seq-1)),lineText,
                      font=setFont, fill=fillColor, direction=None)
...
make_image("【冯柳、邓晓峰等私募大佬持仓最新动向浮出水面】私募大佬尽管喜好各有不同，但大都偏爱中小市值股票。其中，高毅资产邓晓峰今年二季度加仓了旭升股份、西部矿业；高毅资产冯柳新进买入了西部超导，并坚定持仓海康威视。此外，敦和资产对迪普科技保持了一贯的重仓操作")

# make_image("【冯柳、邓晓峰等私募大佬持仓最新动向浮出水面】")